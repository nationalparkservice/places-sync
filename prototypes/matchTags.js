...
var matchTags = function (gisSource, database) {
  return new Bluebird(function (fulfill, reject) {
    ...
    var taskList = [{
      ...
      'params': [gisSource, '{{Get Translations}}', '{{Get Datatype}}']
    }, {
...
var getDatatype = function (sourceType, tagSources) {
  return new Bluebird(function (fulfill, reject) {
    tagSources.datatypes.getRow(sourceType).then(function (datatypes) {
...
var createTagSourceTemplate = function (name, path, primaryKey) {
  return {
    ...
    'tagSource': name,
    'data': __dirname + '/tagTranslations/' + path,
    'format': 'csv',
    'extractionType': 'file',
...
var translationsToQuery = tools.syncPromise(function (gisSource, translations, datatype) {
  ...
  var defaultTags = parseJson(datatype, {});
  var queryString;
  ...
  queryString += 'WHERE LOWER("INPLACES") = \'yes\';'; // TODO where editdate > or something
...
var db = require('../src/createDatabase')();
var test = function () {
  return new Bluebird(function (fulfill, reject) {
    db.addData({
      'name': 'test',
      'data': __dirname + '/tagTranslations/' + 'exampleData.json',
      'format': 'geojson',
      'extractionType': 'file',
      'placesDatatype': 'poi',
      'primaryKey': 'GEOMETRYID'
    }).then(function (gisSource) {
      matchTags(gisSource, db).then(function (r) {
        fulfill(r);
      }).catch(reject);
    }).catch(reject);
  });
